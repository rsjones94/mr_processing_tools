function [ coreg_doneflag ] = ASE_coreg_slw_v2( input, output_nii, reference, omat, omat_status, dcm2nii64_path, reorient_path, flirt_path, flirt_options, bet_path, bet_input_options,bet_ref_options, bet_input, bet_reference )
%SLW wrote this function to put all the coregistration options I care about
%into a matlab function so I can stop messing with the BASH function call
%over and over.
...
%%%%% Have to do this because we switched to sagittal slices, and the
%%%%% different orientation confuses BET and FLIRT
system([reorient_path,' ',input,' ',input])
system([reorient_path,' ',reference,' ',reference])
...
if bet_input    
    ...
    if isempty(exists)
        system([bet_path,' ',input,' ',bet_name,' ',bet_input_options]); %% This triggers FSLOUTPUT error
    end
    input=bet_name;
end
...
if bet_reference
    ...
    [~,exists]=system(['find ',coregdir,' -maxdepth 1 -name ',basename,'_BET',ext]);
    if isempty(exists)
        system([bet_path,' ',reference,' ',bet_name,' ',bet_ref_options]);
    end
    reference=bet_name;
end
%%%%%%%%%%%%%%%%%%%
