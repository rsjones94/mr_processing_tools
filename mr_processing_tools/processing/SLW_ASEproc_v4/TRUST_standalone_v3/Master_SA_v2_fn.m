function [ trust_finflag ] = Master_SA_v2_fn( trustDir,hct )
% trustDir must point to a directory with the patient name, conatinaing an
% Acquired directory. mrid/Acquired/trustfiles. The trust files must be
% inside of the Acquired directory. The trust files must contain
% *SOURCE*TRUST_VEIN* to be identified by the program. Acquired files must
% be PARRECs
...
%     trustDir='/Users/spencerwaddle/Documents/PPE/Scanning/20210301_Donahue_241665';
    [~,mrid]=system(['basename ',trustDir]);
    mrid=mrid(1:end-1); %%% Because unix puts a \n at the end
    trustflip=1;
...
%     trust_reply = questdlg('Does the decay curve look okay?','Saving TRUST Results','Yes','No','Yes');
    ...
    if userinp=='y'
        dataFile=fopen([dataDir,'/','TRUSTresults.txt'],'w');
        try
    ...
    trust_finflag='TRUST has finished processing';
